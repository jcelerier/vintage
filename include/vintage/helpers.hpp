        ...
        float opt)
{
    ...
    {
        if constexpr(requires { eff.midi_input(std::declval<const vintage::MidiEvent&>()); })
        {
...
struct Controls
{
    static const constexpr int32_t parameter_count = boost::pfr::tuple_size_v<decltype(T::parameters)>;
    ...
    {
      [this, &implementation]<std::size_t... Index>(std::integer_sequence<std::size_t, Index...>)
      {
...
struct SynthControls : Controls<T>
{
    ...
    void display(Effect_T& effect, int index, void* ptr)
    {
        ...
        default:
            ...
            break;
        case Controls<T>::parameter_count:
            snprintf(reinterpret_cast<char*>(ptr), vintage::Constants::ParamStrLen, "%d", int(effect.controls.unison_voices.load() * 20));
            break;
        case Controls<T>::parameter_count + 1:
            snprintf(reinterpret_cast<char*>(ptr), vintage::Constants::ParamStrLen, "%.2f", effect.controls.unison_detune.load());
            break;
        case Controls<T>::parameter_count + 2:
            snprintf(reinterpret_cast<char*>(ptr), vintage::Constants::ParamStrLen, "%.2f", effect.controls.unison_volume.load());
            break;
        }
    }
};
...
struct Processor
{
    ...
    void init(Effect_T& effect)
    {
        ...
        if constexpr (effect_processor<float, Effect_T> || synth_processor<float, Effect_T>)
        {
        ...
        if constexpr (effect_processor<double, Effect_T> || synth_processor<double, Effect_T>)
        {
